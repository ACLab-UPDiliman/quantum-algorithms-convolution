...
#Description: This work is inspired by the work in Curtis2004 on application of quantum Fourier transform
#             to 2D template matching problem. This work applies the same method to the approximate string
#             matching problem.
...
# dft(x)
# desc: compute the discrete Fourier transform of a vector of values
# param: x - the vector of values to transform
# return: X - the vector of transformed values
def dft(x):
    X = empty(len(x), dtype=complex)
    for i in xrange(0, len(x)):  # in 0 .. L-1
        print "--------------------"
        cycles_per_sample = double(i) / len(x)  # in 0/L, 1/L, ..., L-1/L
...
# idft(X)
# desc: compute the inverse discrete Fourier transform of a vector of values
# param: X - the vector of values to transform
# return: x - the vector of transformed values
def idft(X):
    x = empty(len(X), dtype=complex)
    for i in xrange(0, len(X)):  # 0 .. N-1
        print "--------------------"
        cycles_per_sample = double(i) / len(X)  # 0/L, 1/L, ..., L-1/L
        print "i:", i, " cycles per sample:", i, "/", len(X), "=", cycles_per_sample
        cos_correlation = 0
        sin_correlation = 0
        for (index, value) in enumerate(X):  # values of input x
            cos_correlation = cos_correlation + ((value.real * cos(2 * pi * index * cycles_per_sample)) +
                                                 (value.imag * sin(2 * pi * index * cycles_per_sample)))
            sin_correlation = sin_correlation - ((value.real * sin(2 * pi * index * cycles_per_sample)) +
                                                 (value.imag * cos(2 * pi * index * cycles_per_sample)))
            print "index:", index, " value:", value, " cos:", cos_correlation, " sin:", sin_correlation
        x[i] = complex(cos_correlation, sin_correlation)
        print "--------------------"
    x /= sqrt(len(X))
...
# normalize(a)
# desc: normalize the values of vector a
# param: a - the vector of values to normalize
# return: a_norm - the vector of normalized values
def normalize(a):
...
# save_file(path, ext, close, verbose)
# desc: save the pyplot to a file in local disk
# param: path - the path where to the file will be saved
# param: ext - the extension of the file to save
# param: close - a boolean value indicating whether the pyplot will be closed or not
# param: verbose - a boolean value indicating whether a log will be printed during operation or not
...
rects2 = subplot.bar([left for left in xrange(0, len(X_normalized_magnitudes))], X_normalized_magnitudes, width,
...
rects1 = subplot.bar([left - 0.2 for left in range(0, len(X), 1)], X_cosinusoid_correlations, width, color="blue",
                     alpha=1)
rects2 = subplot.bar([left - 0.1 for left in range(0, len(X), 1)], X_normalized_cosinusoid_correlations, width,
                     color="blue", alpha=0.5)
rects3 = subplot.bar([left + 0.1 for left in range(0, len(X), 1)], X_sinusoid_correlations, width, color="red", alpha=1)
rects4 = subplot.bar([left + 0.2 for left in range(0, len(X), 1)], X_normalized_sinusoid_correlations, width,
                     color="red", alpha=0.5)
...
rects2 = subplot.bar([left - 0.10 for left in range(0, len(x_normalized_cosinusoid_correlations), 1)],
                     x_normalized_cosinusoid_correlations, width, color="green", alpha=1)
rects3 = subplot.bar([left for left in range(0, len(x_normalized_sinusoid_correlations), 1)],
                     x_normalized_sinusoid_correlations, width, color="red", alpha=1)
...
width = 0.10
subplot = pyplot.subplot(111)
rects1 = subplot.bar([left - 0.05 for left in range(0, len(Y), 1)], Y_cosinusoid_correlations, width, color="blue",
                     alpha=1)
rects2 = subplot.bar([left + 0.05 for left in range(0, len(Y), 1)], Y_sinusoid_correlations, width, color="red",
                     alpha=1)
...
subplot = pyplot.subplot(111)
rects1 = subplot.bar([left - 0.20 for left in range(0, len(Z), 1)], Z_cosinusoid_correlations, width, color="blue",
                     alpha=1)
rects2 = subplot.bar([left - 0.10 for left in range(0, len(Z), 1)], Z_sinusoid_correlations, width, color="red",
                     alpha=1)
rects3 = subplot.bar([left + 0.10 for left in range(0, len(Z), 1)], Z_normalized_cosinusoid_correlations, width,
                     color="blue", alpha=.5)
rects4 = subplot.bar([left + 0.20 for left in range(0, len(Z), 1)], Z_normalized_sinusoid_correlations, width,
                     color="red", alpha=.5)
...
subplot = pyplot.subplot(111)
# plot legend
rects1 = subplot.bar([left - 0.30 for left in range(0, len(z), 1)], z_cosinusoid_correlations, width, color="blue",
                     alpha=1)
rects2 = subplot.bar([left - 0.20 for left in range(0, len(z), 1)], z_sinusoid_correlations, width, color="red",
                     alpha=1)
rects3 = subplot.bar([left - 0.10 for left in range(0, len(z), 1)], (z_cosinusoid_correlations ** 2), width,
                     color="green", alpha=1)
rects4 = subplot.bar([left + 0.10 for left in range(0, len(z), 1)], z_normalized_cosinusoid_correlations, width,
                     color="blue", alpha=.5)
rects5 = subplot.bar([left + 0.20 for left in range(0, len(z), 1)], z_normalized_sinusoid_correlations, width,
                     color="red", alpha=.5)
rects6 = subplot.bar([left + 0.30 for left in range(0, len(z), 1)], (z_normalized_cosinusoid_correlations ** 2), width,
                     color="green", alpha=.5)
subplot.legend((rects1[0], rects2[0], rects3[0], rects4[0], rects5[0], rects6[0]),
               ('z Cos', 'z Sin', 'z Prob', 'z Cos normed', 'z Sin normed', 'z Prob normed'))
subplot.axis([-1, 8, 0, 1])
